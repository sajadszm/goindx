...
class TLC_Plugin {
    ...
    public function register_rest_routes() {
        $controller = new TLC_REST_API_Controller();
    ...
    public function process_telegram_update( $update ) {
        ...
        $message_data_to_insert = array(
            ...
            'message_content'   => $sanitized_agent_message_text,
    ...
    public function handle_visitor_message() {
        ...
        if ($is_new_session) {
            ...
            if (!empty($webhook_url_start) && $session_data_for_webhook) {
                ...
                $this->send_webhook($webhook_url_start, $payload);
            }
        }
        ...
        $session_info_for_telegram = $wpdb->get_row($wpdb->prepare("SELECT visitor_name, visitor_email, referer, utm_source, utm_medium, utm_campaign FROM $sessions_table WHERE session_id = %d", $session_id));
        $visitor_details_array = array("\n\nVisitor Info:", "IP: " . $visitor_ip);
        ...
        if ($wp_user_id > 0) {
            ...
            if ($user_info) $visitor_details_array[] = "User: " . esc_html($user_info->display_name) . " (" . esc_html($user_info->user_email) . ")";
        }
        if ($session_info_for_telegram) {
            if (!empty($session_info_for_telegram->referer)) $visitor_details_array[] = "Referer: " . esc_html($session_info_for_telegram->referer);
            if (!empty($session_info_for_telegram->utm_source)) $visitor_details_array[] = "UTM Source: " . esc_html($session_info_for_telegram->utm_source);
            if (!empty($session_info_for_telegram->utm_medium)) $visitor_details_array[] = "UTM Medium: " . esc_html($session_info_for_telegram->utm_medium);
            if (!empty($session_info_for_telegram->utm_campaign)) $visitor_details_array[] = "UTM Campaign: " . esc_html($session_info_for_telegram->utm_campaign);
        }
        $telegram_message_content .= implode("\n", $visitor_details_array);
        ...
        foreach ( $target_chat_ids as $chat_id ) {
            ...
            $response = $telegram_api->send_message( $chat_id, $telegram_message_content, 'HTML' );
            if ( is_wp_error( $response ) ) {
                error_log( TLC_PLUGIN_PREFIX . 'Failed to send message to Telegram Chat ID ' . $chat_id . ': ' . $response->get_error_message() );
                $success_sending_to_all_targets = false;
            }
        }

        if ( ! $success_sending_to_all_targets ) {
            error_log(TLC_PLUGIN_PREFIX . 'One or more Telegram notifications failed for message ID: ' . $db_message_id);
        }
    ...
    private function get_visitor_ip() {
        ...
        if ( $ip && false !== strpos( $ip, ':' ) && substr_count( $ip, '.' ) === 3 ) { $ip = explode( ':', $ip )[0]; }
    ...
    public function is_currently_within_work_hours() {
        ...
        $open_time = $day_settings['open']; $close_time = $day_settings['close'];
    ...
    public function handle_upload_chat_file() {
        ...
        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( $_POST['nonce'] ), 'tlc_upload_chat_file_nonce' ) ) {
            wp_send_json_error( array( 'message' => __( 'Nonce verification failed.', 'telegram-live-chat' ) ), 403 ); return; }
        if ( !get_option( TLC_PLUGIN_PREFIX . 'file_uploads_enable', false ) ) {
            wp_send_json_error( array( 'message' => __( 'File uploads are disabled.', 'telegram-live-chat' ) ), 403 ); return; }
        if ( empty( $_FILES['chat_file'] ) ) {
            wp_send_json_error( array( 'message' => __( 'No file uploaded.', 'telegram-live-chat' ) ), 400 ); return; }

        $file = $_FILES['chat_file'];
        $visitor_token = isset( $_POST['visitor_token'] ) ? sanitize_text_field( $_POST['visitor_token'] ) : '';
        $current_page = isset( $_POST['current_page'] ) ? esc_url_raw( $_POST['current_page'] ) : '';
        if ( empty( $visitor_token ) ) {
            wp_send_json_error( array( 'message' => __( 'Visitor token required.', 'telegram-live-chat' ) ), 400 ); return; }

        $max_size_mb = absint(get_option(TLC_PLUGIN_PREFIX . 'file_uploads_max_size_mb', 2));
        $max_size_bytes = $max_size_mb * 1024 * 1024;
        if ($file['size'] > $max_size_bytes) {
            wp_send_json_error( array( 'message' => sprintf(__('File too large (max %d MB).', 'telegram-live-chat'), $max_size_mb) ), 400 ); return; }
        ...
        if (!empty($allowed_types_str)) {
            ...
            foreach ( $allowed_exts_normalized as $ext_normalized ) {
                foreach ( get_allowed_mime_types() as $exts_patterns => $mime_type_val ) {
                    if ( preg_match( '/\b' . preg_quote( $ext_normalized, '/' ) . '\b/i', $exts_patterns ) ) $mimes[ $exts_patterns ] = $mime_type_val; } }
            if (empty($mimes) && $allowed_types_str !== '') {
                 wp_send_json_error( array( 'message' => __( 'Invalid allowed file types configured.', 'telegram-live-chat' ) ), 400 ); return; }
        }
        ...
        $upload_overrides = array( 'test_form' => false, 'mimes' => $mimes,
            'unique_filename_callback' => function($dir, $name, $ext) { return hash('sha1', sanitize_file_name($name) . microtime()) . $ext; });
        $custom_upload_dir_filter_func = function( $pathdata ) use ( $tlc_visitor_upload_path, $tlc_visitor_upload_url ) {
            $pathdata['path'] = $tlc_visitor_upload_path; $pathdata['url'] = $tlc_visitor_upload_url; return $pathdata; };
        add_filter( 'upload_dir', $custom_upload_dir_filter_func );
        $movefile = wp_handle_upload( $file, $upload_overrides );
        remove_filter( 'upload_dir', $custom_upload_dir_filter_func );

        if ( $movefile && ! isset( $movefile['error'] ) ) {
            $file_url = $movefile['url']; $file_path = $movefile['file']; $original_filename = sanitize_file_name($file['name']);
            $sessions_table = $wpdb->prefix . TLC_PLUGIN_PREFIX . 'chat_sessions';
            $messages_table = $wpdb->prefix . TLC_PLUGIN_PREFIX . 'chat_messages';
            ...
            if (!$session_id) {
                ...
                $session_id = $wpdb->insert_id;
            } else { $wpdb->update( $sessions_table, array( 'last_active_time' => current_time( 'mysql' ) ), array( 'session_id' => $session_id ) );}
            if ( ! $session_id ) { if (file_exists($file_path)) unlink($file_path);
                wp_send_json_error( array( 'message' => __( 'Session error for file.', 'telegram-live-chat' ) ), 500 ); return; }
            $message_content = sprintf(__('File: [%1$s](%2$s)', 'telegram-live-chat'), $original_filename, $file_url);
            $message_inserted = $wpdb->insert( $messages_table, array( 'session_id' => $session_id, 'sender_type' => 'visitor',
                'message_content' => $message_content, 'timestamp' => current_time( 'mysql' ), 'page_url' => $current_page, ));
            $db_message_id = $wpdb->insert_id;
            ...
            if ( !empty( $bot_token_setting ) && (!empty( $admin_user_ids_str ) || !empty($group_chat_id)) ) {
                ...
                $target_chat_ids = array();
                if (!empty($admin_user_ids_str)) $target_chat_ids = array_merge($target_chat_ids, array_map( 'trim', explode( ',', $admin_user_ids_str ) ));
                if (!empty($group_chat_id) && tlc_is_numeric_or_at_sign_string($group_chat_id) ) $target_chat_ids[] = $group_chat_id;
                $target_chat_ids = array_unique(array_filter($target_chat_ids));
                foreach ( $target_chat_ids as $chat_id ) {
                    if (empty($chat_id)) continue;
                    if (!is_numeric($chat_id) && strpos($chat_id, '@') !== 0) { error_log(TLC_PLUGIN_PREFIX . "Skipping invalid chat_id for file send: " . $chat_id); continue; }
        ...
        } else { wp_send_json_error( array( 'message' => $movefile['error'] ), 400 ); }
        die();
    }
    ...
    public function handle_submit_chat_rating() {
        ...
        if ( empty( $visitor_token ) ) { wp_send_json_error( array( 'message' => __( 'Visitor token required.', 'telegram-live-chat' ) ), 400 ); return; }
        if ( $rating < 1 || $rating > 5 ) { wp_send_json_error( array( 'message' => __( 'Invalid rating value.', 'telegram-live-chat' ) ), 400 ); return; }
        $sessions_table = $wpdb->prefix . TLC_PLUGIN_PREFIX . 'chat_sessions';
        ...
        if ( ! $session ) { wp_send_json_error( array( 'message' => __( 'No active session found for this visitor to rate.', 'telegram-live-chat' ) ), 404 ); return; }
        $updated = $wpdb->update( $sessions_table, array( 'rating' => $rating, 'rating_comment' => $comment ),
            array( 'session_id' => $session->session_id ), array( '%d', '%s' ), array( '%d' ) );
        if ( false === $updated ) { wp_send_json_error( array( 'message' => __( 'Could not save rating. Database error.', 'telegram-live-chat' ) ), 500 ); return; }
    ...
    private function send_webhook($url, $payload_data) {
        $secret = get_option(TLC_PLUGIN_PREFIX . 'webhook_secret', '');
