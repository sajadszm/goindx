...
from bot.subscription_handlers import (
    status_command, subscribe_command, payment_method_callback, PAYMENT_METHOD_PREFIX
)
...
async def language_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    current_lang = db_user['language'] if db_user else 'en'
    if not db_user:
        add_user(user.id)
    keyboard = [
        [InlineKeyboardButton("English", callback_data=f"{LANG_CALLBACK_PREFIX}en")],
...
async def language_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        db_user_temp = get_user(user.id)
        error_lang = db_user_temp['language'] if db_user_temp else 'en'
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user or not update.message:
        ...
        return
    db_user = get_user(user.id)
    lang_code = db_user['language'] if db_user else 'en'
    if not db_user:
        add_user(user.id)
...
def register_handlers(application: Application) -> None:
    ...
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_url_message))  # ✅ اصلاح شده
